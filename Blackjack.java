...
public class Blackjack {
    ...
    private static final int SURRENDER = 4;
    private static final int COMPLETED = 5;
    
    private static double money; // Amount of money user has.
    private static double bet;   // Amount user bets on a game.
    private static double minbet; // Minimum bet. Lowest minimum bet will be $1
    private static double blackjackMult; // Blackjack multiplier (3 to 2: 1.5, 6 to 5: 1.2)
    private static boolean didSplit; // Has the user split?
    private static int splitCount; // How many times the user has split
    private static int splitLimit; // number of splits allowed (0 to 4); standard is 1-3; 3 is default
    // private static boolean didSplitAces; // Has the user split aces?
    private static boolean evenMoney; // Did the user opt for even money?
    private static boolean insured; // Did the user take insurance?
    private static Deck deck; // A double deck of cards
    private static int simSittings; // Number of sittings to average in a simulation
    private static int simRounds; // Number of rounds to run in a sitting
    public static ArrayList<String> availableOptions; // List of all available options
    public static ArrayList<String> options; // User options to change odds of the game
    private static int llDouble; // low limit for doubling
    private static int ulDouble; // high limit for doubling
    private static int charlieCount; // number of cards to win by Charlie rule
    private static String betStratOpt; // string to identify the betting strategy to used based on the true count
    
    public static int bestNum; // ideal number for the game
    public static int hitNum; // highest number dealer still has to hit
    public static int surrendNum; // surrender flag (-1 for not set, 1 for early surrender, 2 for late surrender)
    
    private static boolean stratWindow; // flag for whether to display the strategy window to the user for optional simulation strategy changes
    private static int percentComplete; // for testing purposes to see how far the simulation is to completing
    
    // private static int surrenderCount = 0;
...
/*** All the methods for playing blackjack are in this section. ***/
    ...
    static ArrayList<BlackjackHand> playBlackjack() {
    ...
    static boolean canDouble(BlackjackHand h) { // create overrided versions for variants
    ...
    static boolean canSplit(BlackjackHand h) { // create overrided versions for variants
    	if (h.getCardCount() != 2) {
    		return false;
    	} else if (bet > money) {
    		return false;
    ...
    static void printPlayOptions(BlackjackHand h) {
    ...
    static void printResponseOptions(BlackjackHand h) {
    	if (canDouble(h) && canSplit(h) && canLateSurrender(h)) {
    ...
    static void askAndSetOptions() {
    	System.out.println();
        
        String opts = "0";
        do {
        	minbet = 5; // kludge set
			money = 100; // kludge set
    ...
    static boolean validOptions(String opts) {
    	if (opts.matches("[ \\t]*")) {
    		return true;
    		...
    		System.out.println("SM[num] - Set the starting amount of money. e.g. SM300.");
    		System.out.println("          Must be an integer. Lowest amount is $1, highest is $10000.");
    		...
    		System.out.println("User has a stake of $100, with minimum bet $5.");
    ...
    static boolean validOption(String str) {
    	if (availableOptions.contains(str) || str.matches("ND[1-8]")) {
    		return true;
    	} else if (str.matches("MB[1-9]\\d?\\d?\\d?\\d?")) {
    		minbet = Integer.parseInt(str.substring(2));
    		return true;
    	} else if (str.matches("SM[1-9]\\d?\\d?\\d?\\d?")) {
    ...
    static void applyOptions(String opts) {
    ...
    static void applyOption(String str) {
    	if (str.contains("MB")) {
    		minbet = Integer.parseInt(str.substring(2));
    ...
    static void applySpecialOption(String str) {
    	if (str.contains("CC")) {
    		String[] temp = str.split("/");
    		if (temp.length == 1) {
    ...
    static boolean moneyValid() {
    	if (minbet > money || money > 10000) {
    		System.out.println();
    		System.out.println("Invalid option(s).");
			System.out.println();
			return false;
    ...
    static boolean insuranceCheck(BlackjackHand uh, BlackjackHand dh) { // returns true if the round has completed
...
/*** All the methods for simulating blackjack are in this section. ***/
    ...
    static void simulateBlackjack() { // optional: simInsuranceCheck
    	double moneyWon;
    	double moneyLost;
    	...
    	while (true) {
	    	...
	    	Strategy.setStrategy(deck.getNumDecks(), !options.contains("NHS"), hitNum, surrendNum, charlieCount, betStratOpt);
	    	strategyCheck();
	    	favor = 0.0;
	    	...
	    	for (int i = 0; i < simSittings; i++) {
		    	...
		    	for (int j = 0; j < simRounds; j++) {
		    		...
		    		bet = Strategy.getTrueCountBet(5.0, deck.getTrueCount());
			        ...
			        /* Check for early surrender. */
			        if (canEarlySurrender()) {
			        	String surMove = Strategy.getMove(userHand, dealerHand, simCanSplit(userHand));
		...
		//	        }
			        ...
			        */
			        ...
			        while (gameHands.size() > handsPlayed) {
				        ...
				        while (true) {
				                ...
				                if (userHand.getCard(0).getValueAsString().equals("Ace")) { // only one card after splitting aces default
				                	if (!options.contains("DSA") && !options.contains("RSA")) {
				                		...
				                		if (simCanSplit(userHand)) { // dealt another ace
				                			String tempAceMove = Strategy.getMove(userHand, dealerHand, true);
				        	...
				        	String move = Strategy.getMove(userHand, dealerHand, simCanSplit(userHand));
		    	...
		    	/* Current formula for calculating percentage favor. Not sure this is entirely correct. */
    ...
    static boolean simCanDouble(BlackjackHand h) {
    	if (h.getCardCount() != 2 && !options.contains("DAN")) {
    		return false;
    	} else if (didSplit && !options.contains("DAS")) {
    		return false;
    ...
    static boolean simCanSplit(BlackjackHand h) {
    	if (h.getCardCount() != 2) {
    		return false;
    	} else if (splitCount >= splitLimit) {
    		return false;
    ...
    static void simAskAndSetOptions() {
    	System.out.println();
    ...
    static boolean simValidOptions(String opts) { // only use bet increments of $5
    	if (opts.matches("[ \\t]*")) {
    		return true;
    		...
    		System.out.println("Standard bet is $5.");
    		System.out.println("Double deck is used and reshuffled when over half the cards have been dealt.");
    		System.out.println("Blackjack pays 3 to 2.");
    		System.out.println("Dealer hits on soft 17.");
    		System.out.println("No double downs after splits.");
    ...
    static boolean simValidOption(String str) {
    ...
    static void simApplyOptions(String opts) {
    	options = new ArrayList<String>();
    	ArrayList<String> temp = new ArrayList<String>(Arrays.asList(opts.replaceAll("\\s","").split(",")));
    	for (String str : temp) {
    ...
    static void simApplyOption(String str) {
    	if (str.contains("BP")) {
    		String[] temp = str.split(":");
    		if (temp.length == 1) {
    			blackjackMult = 0.0;
    ...
    static void simApplySpecialOption(String str) {
    	if (str.contains("CC")) {
    		String[] temp = str.split("/");
    		if (temp.length == 1) {
    			deck.setCutCard(Integer.parseInt(temp[0].substring(2)), 1);
    ...
    static void strategyCheck() {
    	if (stratWindow) {
    		Strategy.promptStrategyWindow();
    	}
    	Strategy.addStrats();
    }
    
    // not completed
    static boolean simInsuranceCheck(BlackjackHand uh, BlackjackHand dh) { // returns true if the round has completed 
    	return false;
    }
...
/*** All multi-purpose methods are in this section. ***/
    ...
    static boolean isValidBet(String s) { // 50 cent increments from 0 to infinity
        return s.matches("([ \\t]*\\d+(\\.[50]0?)?[ \\t]*)|([ \\t]*\\d*(\\.[50]0?)[ \\t]*)");  
    }  
    
    static boolean isValidIns(String s) { // 50 cent increments from .50 to infinity
        return s.matches("([ \\t]*[1-9]\\d*(\\.[50]0?)?[ \\t]*)|([ \\t]*\\d*(\\.50?)[ \\t]*)");  
    }  

    static boolean isNumericInt(String s) {  
        return s.matches("[ \\t]*\\d+[ \\t]*");  
    }  
    ...
    static boolean canEarlySurrender() {
    	return (surrendNum == 1);
    }
    
    static boolean canLateSurrender(Hand h) {
    	return ((surrendNum == 2) && h.getCardCount() == 2 && !didSplit);
    }
    
    static void percentCheck(int sc, int st, int rc, int rt) { // sc: sittings completed, st: sittings total, rc: rounds completed, rt: rounds total
    	double temp1 = 1.0 * sc;
    	double temp2 = 1.0 * st;
    	double temp3 = 1.0 * rc;
    	double temp4 = 1.0 * rt;
